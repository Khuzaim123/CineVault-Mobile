...
import { ArrowLeftIcon, StarIcon, HeartIcon, BookmarkIcon, PlayIcon, CalendarIcon, ClockIcon } from '../../components/icons';
...
export default function MovieDetailScreen() {
  ...
  const toggleFavorite = async () => {
    ...
    setFavLoading(true);
    try {
      if (isFav) { await favoritesService.remove(user.id, movie.id); setIsFav(false); }
      else { await favoritesService.add(user.id, movie); setIsFav(true); }
  ...
  const toggleWatchlist = async () => {
    ...
    setWlLoading(true);
    try {
      if (isWl) { await watchlistService.remove(user.id, movie.id); setIsWl(false); }
      else { await watchlistService.add(user.id, movie); setIsWl(true); }
    } catch (e) { Alert.alert('Error', e.message); }
  ...
  const poster = tmdb.poster(movie.poster_path, 'w342');
  ...
  return (
    <View style={styles.screen}>
        ...
        <View style={styles.metaRow}>
          ...
          <View style={styles.metaInfo}>
            ...
            <View style={styles.ratingRow}>
              ...
              <Text style={styles.voteCount}>({(movie.vote_count || 0).toLocaleString()})</Text>
            </View>

            {/* Runtime & Release */}
            <View style={styles.detailRow}>
              <CalendarIcon size={13} color={Colors.textMuted} />
              <Text style={styles.detailText}>{movie.release_date?.slice(0, 4) || '—'}</Text>
            </View>
            <View style={styles.detailRow}>
          ...
          <View style={styles.actions}>
            <TouchableOpacity
              ...
              onPress={() => router.push(
                `/player?tmdbId=${movie.id}&mediaType=movie&title=${encodeURIComponent(movie.title)}&trailerKey=${trailer?.key || ''}`
              )}
              activeOpacity={0.85}
            >
          ...
          >
            {favLoading
              ? <ActivityIndicator size={18} color={Colors.crimson} />
              : <HeartIcon size={20} color={isFav ? Colors.crimson : Colors.textPrimary} filled={isFav} />}
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.iconBtn, isWl && styles.iconBtnGold]}
            onPress={toggleWatchlist}
            disabled={wlLoading}
          >
            {wlLoading
              ? <ActivityIndicator size={18} color={Colors.gold} />
              : <BookmarkIcon size={20} color={isWl ? Colors.gold : Colors.textPrimary} filled={isWl} />}
          </TouchableOpacity>
        </View>
          ...
          <View style={styles.section}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.castScroll}>
              {cast.map((actor) => (
                ...
                >
                  <Image
                    source={{ uri: tmdb.profile(actor.profile_path) || 'https://via.placeholder.com/80x100' }}
      ...
      <AuthPromptModal
        ...
        message="Sign in to add to your Favorites and Watchlist."
      />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  backdropContainer: { height: BACKDROP_H, position: 'relative' },
  ...
  metaRow: {
    ...
    marginTop: -60, gap: 14, alignItems: 'flex-end',
  },
  poster: {
    width: 110, height: 165, borderRadius: Radius.md,
  ...
  detailText: { color: Colors.textMuted, fontFamily: Fonts.body, fontSize: 13 },
  genres: { flexDirection: 'row', flexWrap: 'wrap', gap: 5, marginTop: 6 },
  genreChip: {
  ...
  playBtn: {
    ...
    gap: 8, backgroundColor: Colors.gold, borderRadius: Radius.md, paddingVertical: 12,
  },
  ...
  actorPhoto: {
    width: 72, height: 96, borderRadius: Radius.md,
